...
export const demoProfiles: Record<UserRole, Profile> = {
  student: { id: 'demo-student-1', name: '演示学生-小雨', role: 'student', school_id: 'demo-school-1', class_id: 'class-7a', student_no: 'S001' },
  teacher: { id: 'demo-teacher-1', name: '演示教师-陈老师', role: 'teacher', school_id: 'demo-school-1', class_id: 'class-7a', student_no: null },
  admin: { id: 'demo-admin-1', name: '演示管理员', role: 'admin', school_id: null, class_id: null, student_no: null }
};

export let demoSchools = [
  { id: 'demo-school-1', name: '黄石实验学校', created_at: new Date().toISOString() }
];

export let demoClasses = [
  { id: 'class-7a', school_id: 'demo-school-1', name: '七年级 1 班', grade: '七年级', teacher_id: 'demo-teacher-1', invite_code: 'HS701', created_at: new Date().toISOString() },
  { id: 'class-7b', school_id: 'demo-school-1', name: '七年级 2 班', grade: '七年级', teacher_id: 'demo-teacher-1', invite_code: 'HS702', created_at: new Date().toISOString() },
  { id: 'class-8a', school_id: 'demo-school-1', name: '八年级 1 班', grade: '八年级', teacher_id: 'demo-teacher-1', invite_code: 'HS801', created_at: new Date().toISOString() }
];
...
export let demoNotifications = [
  { id: 'n-1', user_id: 'demo-student-1', type: 'review', title: '教师已批改学习档案', content: '你有一条新的教师评价，请到学习档案查看。', read: false, created_at: new Date().toISOString() }
];
...
export function getDemoUserIdFromCookie(cookieHeader?: string | null) {
  if (!cookieHeader) return null;
  const raw = cookieHeader.split(';').map((x) => x.trim()).find((x) => x.startsWith('demo_user_id='));
